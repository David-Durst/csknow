...
def create_non_confounding_test_plant_states():
    ...
    make_index_column(rounds_without_confounds_df)
    rounds_without_confounds_df.rename({'index': 'non_confound_index'}, axis=1, inplace=True)
    ...
    non_confounding_test_plant_states.sort_values(by='non_confound_index', inplace=True)
