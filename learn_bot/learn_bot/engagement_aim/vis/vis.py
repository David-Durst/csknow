...
def vis(all_data_df: pd.DataFrame, pred_df: pd.DataFrame = None):
    ...
    if pred_df is None:
        ...
        pred_ax_objs = AxObjs(True, fig, pred_pos_ax, pred_speed_ax, first_tick_columns,
                              engine_columns, cur_tick_columns)
    setPosAxSettings(input_pos_ax, "Input Aim")
    setSpeedAxSettings(input_speed_ax, "Input Aim")
    input_ax_objs = AxObjs(False, fig, input_pos_ax, input_speed_ax, first_tick_columns,
                           engine_columns, cur_tick_columns)
