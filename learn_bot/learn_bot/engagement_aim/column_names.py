from learn_bot.libs.temporal_column_names import TemporalIOColumnNames, get_temporal_field_str, get_base_field_str
...
base_learning_float_delta_columns: List[str] = [
    "attacker eye pos x", "attacker eye pos y", "attacker eye pos z",
    "victim eye pos x", "victim eye pos y", "victim eye pos z",
]

base_learning_float_180_standard_angle_columns: List[str] = [
    "scaled recoil angle x",
    #"victim min view angle x",
    #"victim max view angle x",
    #"victim cur head view angle x",
]

base_learning_float_180_delta_angle_columns: List[str] = [
...
base_learning_float_90_standard_angle_columns: List[str] = [
...
base_learning_float_90_delta_angle_columns: List[str] = [
...
temporal_io_float_180_standard_angle_column_names = \
    TemporalIOColumnNames(base_learning_float_180_standard_angle_columns, PRIOR_TICKS, CUR_TICK, FUTURE_TICKS)
temporal_io_float_180_delta_angle_column_names = \
    TemporalIOColumnNames(base_learning_float_180_delta_angle_columns, PRIOR_TICKS, CUR_TICK, FUTURE_TICKS)
input_float_180_delta = \
    [DeltaColumn(c_rel, get_temporal_field_str(base_changed_offset_coordinates.victim_aabb_head_x, -1), None)
     for c_rel in temporal_io_float_180_delta_angle_column_names.past_columns]
all_float_180_delta = \
    [DeltaColumn(c_rel, get_temporal_field_str(base_changed_offset_coordinates.victim_aabb_head_x, -1), None)
     for c_rel in temporal_io_float_180_delta_angle_column_names.all_columns]

temporal_io_float_90_standard_angle_column_names = \
    TemporalIOColumnNames(base_learning_float_90_standard_angle_columns, PRIOR_TICKS, CUR_TICK, FUTURE_TICKS)
temporal_io_float_90_delta_angle_column_names = \
    TemporalIOColumnNames(base_learning_float_90_delta_angle_columns, PRIOR_TICKS, CUR_TICK, FUTURE_TICKS)
input_float_90_delta = \
    [DeltaColumn(c_rel, get_temporal_field_str(base_changed_offset_coordinates.victim_aabb_head_y, -1), None)
     for c_rel in temporal_io_float_90_delta_angle_column_names.past_columns]
all_float_90_delta = \
    [DeltaColumn(c_rel, get_temporal_field_str(base_changed_offset_coordinates.victim_aabb_head_y, -1), None)
     for c_rel in temporal_io_float_90_delta_angle_column_names.all_columns]
...
input_column_types = ColumnTypes(temporal_io_float_standard_column_names.past_columns, input_float_delta,
                                 temporal_io_float_180_standard_angle_column_names.past_columns, input_float_180_delta,
                                 temporal_io_float_90_standard_angle_column_names.past_columns, input_float_90_delta,
                                 ...
                                 [], [],
                                 temporal_io_float_180_standard_angle_column_names.past_columns +
                                 temporal_io_float_180_delta_angle_column_names.past_columns)

all_time_column_types = ColumnTypes(temporal_io_float_standard_column_names.all_columns, all_float_delta,
                                    temporal_io_float_180_standard_angle_column_names.all_columns, all_float_180_delta,
                                    temporal_io_float_90_standard_angle_column_names.all_columns, all_float_90_delta,
                                    ...
                                    [], [],
                                    temporal_io_float_180_standard_angle_column_names.all_columns +
                                    temporal_io_float_180_delta_angle_column_names.all_columns)

output_relative_180_cols = \
    temporal_io_float_180_delta_angle_column_names.get_matching_cols(
        base_changed_offset_coordinates.attacker_x_view_angle, False, True, True)
output_relative_90_cols = \
    temporal_io_float_90_delta_angle_column_names.get_matching_cols(
        base_changed_offset_coordinates.attacker_y_view_angle, False, True, True)
output_ref_180_col = get_temporal_field_str(base_changed_offset_coordinates.attacker_x_view_angle, -1)
output_ref_90_col = get_temporal_field_str(base_changed_offset_coordinates.attacker_y_view_angle, -1)
output_target_180_cols = \
    temporal_io_float_180_delta_angle_column_names.get_matching_cols(
        base_changed_offset_coordinates.victim_aabb_head_x, False, True, True)
output_target_90_cols = \
    temporal_io_float_90_delta_angle_column_names.get_matching_cols(
        base_changed_offset_coordinates.victim_aabb_head_y, False, True, True)
output_delta_180 = [DeltaColumn(c_rel, output_ref_180_col, c_target) for c_rel, c_target in
                  zip(output_relative_180_cols, output_target_180_cols)]
output_delta_90 = [DeltaColumn(c_rel, output_ref_90_col, c_target) for c_rel, c_target in
                  zip(output_relative_90_cols, output_target_90_cols)]
...
num_x_targets = len(output_target_180_cols)

#output_column_types = ColumnTypes(output_standard_cols, output_delta, [], [])
output_column_types = ColumnTypes([], [], [], output_delta_180, [], output_delta_90,
