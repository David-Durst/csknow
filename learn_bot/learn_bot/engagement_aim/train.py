    ...
    def train_or_test_SL_epoch(dataloader, model, optimizer, first_epoch, last_epoch, blend_amount, train=True):
        ...
        with tqdm(total=len(dataloader), disable=False) as pbar:
            for batch, (X, Y, targets, attacking, all_time_X, idxs) in enumerate(dataloader):
                ...
                # since row_rollout is one tick at a time, need to encode all delta values as relative to prior
                transformed_Y = column_transformers.transform_columns(False, Y, X, True)
