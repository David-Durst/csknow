...
from learn_bot.old_bot_models.baseline import *
from joblib import dump
from learn_bot.old_bot_models.sequence_model import SequenceNeuralNetwork, SNNArgs
from learn_bot.old_bot_models.sequence_dataset import SequenceBotDatasetArgs, SequenceBotDataset
from dataclasses import dataclass
import matplotlib.pyplot as plt
from learn_bot.old_bot_models.sequence_creation import organize_into_sequences
...
plt.suptitle('Predicted Mouse Change Given Game State, Prior Tick Mouse Changes')
all_data_df.hist('delta view x 1', ax=axs[0,0], bins=100)
axs[0,0].set_title('Delta X, 1 Prior Tick')
all_data_df.hist('delta view y 1', ax=axs[0,1], bins=100)
axs[0,1].set_title('Delta Y, 1 Prior Tick')
all_data_df.hist('delta view x 4', ax=axs[1,0], bins=100)
axs[1,0].set_title('Delta X, 4 Prior Ticks')
all_data_df.hist('delta view y 4', ax=axs[1,1], bins=100)
axs[1,1].set_title('Delta Y, 4 Prior Ticks')
all_data_df.hist('delta view x 8', ax=axs[2,0], bins=100)
axs[2,0].set_title('Delta X, 8 Prior Ticks')
all_data_df.hist('delta view y 8', ax=axs[2,1], bins=100)
axs[2,1].set_title('Delta Y, 4 Prior Ticks')
plt.tight_layout()
plt.show()

fig, axs = plt.subplots(3,2)
plt.suptitle('Predicted Mouse Position')
