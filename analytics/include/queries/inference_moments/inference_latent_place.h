...
#ifndef CSKNOW_INFERENCE_LATENT_PLACE_H
#define CSKNOW_INFERENCE_LATENT_PLACE_H
...
namespace csknow::inference_latent_place {
    ...
    class InferenceLatentPlaceResult : public QueryResult {
    public:
        ...
        vector<PlaceIndex> placeIndex;
        vector<array<float, csknow::feature_store::num_places>> playerPlaceProb;
        IntervalIndex placesPerTick;


        InferenceLatentPlaceResult(DistanceToPlacesResult & distanceToPlacesResult) {
            variableLength = true;
            startTickColumn = 0;
            perEventLengthColumn = 2;
            havePlayerLabels = true;
            playerLabels = distanceToPlacesResult.places;
        ...
        void oneLineToCSV(int64_t index, std::ostream &s) override {
            s << index << "," << startTickId[index] << "," << endTickId[index] << "," << tickLength[index]
              << "," << playerId[index] << "," << placeIndex[index] << std::endl;
        }
        ...
        void toHDF5Inner(HighFive::File &file) override {
            ...
            file.createDataSet("/data/place index", placeIndex, hdf5FlatCreateProps);
        }
...
#endif //CSKNOW_INFERENCE_LATENT_PLACE_H
