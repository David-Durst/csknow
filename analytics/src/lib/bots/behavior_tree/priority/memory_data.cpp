...
void EnemyPositionsMemory::updatePositions(const ServerState & state, double maxMemorySeconds) {
    ...
    for (const CSGOId player : srcPlayers) {
        if (state.getClient(player).isAlive) {
            const auto visibleEnemies = state.getVisibleEnemies(player);
    ...
    for (const auto & [id, position] : positions) {
        double timeSinceSeen = ServerState::getSecondsBetweenTimes(position.lastSeenTime, state.loadTime);
