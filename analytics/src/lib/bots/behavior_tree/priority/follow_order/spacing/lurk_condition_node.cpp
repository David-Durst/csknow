...
namespace follow::spacing {
    bool LurkConditionNode::valid(const ServerState &state, TreeThinker &treeThinker) {
        ...
        bool readyToExecute = blackboard.strategy.playerToWaypointIndex[treeThinker.csgoId] > 0;
        return readyToExecute && blackboard.teamToLastRoundSawEnemy[curClient.team] != state.roundNumber;
    }
}