...
int main(int argc, char * argv[]) {
    if (argc != 5) {
        ...
        std::cout << "4. 0 if all round ids, 1 if bot good round ids, 2 if human good round ids" << std::endl;
        return 1;
    }
    ...
    int goodRoundIdsType = std::stoi(argv[4]);
    ...
    std::optional<reference_wrapper<const set<int64_t>>> goodRoundIds;
    if (goodRoundIdsType == 1) {
        goodRoundIds = botGoodRounds;
    }
    else if (goodRoundIdsType == 2) {
        goodRoundIds = humanGoodRounds;
    }
    csknow::multi_trajectory_similarity::TraceSimilarityResult traceSimilarityResult(predictedTraces, groundTruthTraces,
                                                                                     goodRoundIds, goodRoundIds);
