...
int main(int argc, char * argv[]) {
    if (argc != 6 && argc != 7) {
        std::cout << "please call this code 6 arguments: " << std::endl;
        ...
        std::cout << "4. predicted good rounds - 0 if all round ids, 1 if bot good round ids, 2 if small human good round ids, 3 if train round ids" << std::endl;
        std::cout << "5. ground truth good rounds - 0 if all round ids, 1 if bot good round ids, 2 if small human good round ids, 3 if train round ids" << std::endl;
        std::cout << "6. (optional) path to train test split dir" << std::endl;
        return 1;
    }
    ...
    string trainTestSplitDir;
    if (argc == 7) {
        trainTestSplitDir = argv[6];
    }
    ...
    // hacky holders since the actual round ids are optional references
    set<int64_t> predictedTrainRoundIds, groundTruthTrainRoundIds;
    ...
    else if (predictedGoodRoundIdsType == 3) {
        predictedTrainRoundIds = load_one_data_file_train_round_ids(trainTestSplitDir, predictedPathStr);
        predictedGoodRoundIds = predictedTrainRoundIds;
    }
    ...
    else if (groundTruthGoodRoundIdsType == 3) {
        groundTruthTrainRoundIds = load_one_data_file_train_round_ids(trainTestSplitDir, groundTruthPathStr);
        groundTruthGoodRoundIds = groundTruthTrainRoundIds;
    }
