...
def run():
    ...
    for f in fs.glob(s3path_str(BIG_TRAIN_PROCESSED_PROS / "*", False)):
        fs.mv(f, s3path_str(BIG_TRAIN_UNPROCESSED_PROS, True))
    for f in fs.glob(s3path_str(BIG_TRAIN_PROCESSED_BOTS / "*", False)):
        fs.mv(f, s3path_str(BIG_TRAIN_UNPROCESSED_BOTS, True))

    # clear the csv data
    s3_resource = boto3.resource('s3')
    bucket = s3_resource.Bucket(BUCKET.bucket)
    bucket.objects.filter(Prefix=BIG_TRAIN_CSVS.key).delete()
