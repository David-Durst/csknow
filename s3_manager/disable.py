...
def run():
    ...
    for f in fs.glob(s3path_str(MANUAL_PROCESSED_PROS / "*", False)):
        fs.mv(f, s3path_str(MANUAL_DISABLED, True))
    for f in fs.glob(s3path_str(MANUAL_PROCESSED_BOTS / "*", False)):
        fs.mv(f, s3path_str(MANUAL_DISABLED, True))
    for f in fs.glob(s3path_str(MANUAL_UNPROCESSED_PROS / "*", False)):
        fs.mv(f, s3path_str(MANUAL_DISABLED, True))
    for f in fs.glob(s3path_str(MANUAL_UNPROCESSED_BOTS / "*", False)):
        fs.mv(f, s3path_str(MANUAL_DISABLED, True))
