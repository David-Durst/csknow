...
def run():
    ...
    for f in fs.glob(s3path_str(ROLLOUT_PROCESSED_PROS / "*", False)):
        fs.mv(f, s3path_str(ROLLOUT_UNPROCESSED_PROS, True))
    for f in fs.glob(s3path_str(ROLLOUT_PROCESSED_BOTS / "*", False)):
        fs.mv(f, s3path_str(ROLLOUT_UNPROCESSED_BOTS, True))

    # clear the csv data
    s3_resource = boto3.resource('s3')
    bucket = s3_resource.Bucket(BUCKET.bucket)
    bucket.objects.filter(Prefix=ROLLOUT_CSVS.key).delete()
